...
const noise = seed => {
  const value =
    Math.sin(seed * 127.1 + 311.7) * 43758.5453;
...
const buildGrid = ({
  ...
  if (columns * rows > MAX_PIXELS) {
    size = Math.ceil(
      size *
        Math.sqrt((columns * rows) / MAX_PIXELS)
    );
...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    const total =
      Math.max(200, settings.duration);
