<div class="w-full bg-white font-sans overflow-x-hidden selection:bg-[#ccff00] selection:text-black bg-grid-pattern relative min-h-screen flex flex-col justify-between">
    ...
    <section class="space-y-8 w-full my-4">
        ...
        {{-- Filter Kategori --}}
        <div class="rimesa-container flex items-center justify-start sm:justify-center gap-2 overflow-x-auto pb-4 mb-8 no-scrollbar">
            <button wire:click="setCategory('')" 
                    class="px-3.5 py-1.5 sm:px-5 sm:py-2 border-2 text-xs sm:text-sm font-display whitespace-nowrap transition-all duration-200 cursor-pointer
        ...
        <div class="rimesa-container rimesa-grid items-stretch" wire:loading.class="opacity-60">
                    ...
                    <a href="{{ route('work.show', $work->slug ?? $work->id) }}" class="block w-full">
                        <div class="relative w-full aspect-square bg-gray-900 border-2 border-[#FC00BB] shrink-0">
                            ...
                            <div class="w-full h-full overflow-hidden flex items-center justify-center">
                                @if ($work->cover_image)
                                    <img src="{{ asset('storage/' . $work->cover_image) }}" alt="{{ $work->title }}" loading="lazy" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                @else
            ...
            @empty
                <div class="col-span-full text-center py-16 text-[#2F3AFF]">
                    <p class="font-display text-lg sm:text-xl tracking-wider">Belum ada rekomendasi karya.</p>
                </div>
            @endforelse
        </div>

        <div class="text-center pt-4">
            <a href="{{ route('explore.more', ['category' => $wasteType]) }}" class="inline-block text-[#2F3AFF] hover:text-[#FC00BB] font-display text-sm sm:text-xl transition tracking-wider underline underline-offset-4">
                Lihat lebih banyak →
            </a>
        </div>
    </section>
    ...
    {{-- ============================================================ --}}
    {{-- SECTION-SECTION LAIN — tetap dibatasi max-w-7xl seperti semula --}}
    {{-- ============================================================ --}}
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20 sm:space-y-28 w-full my-4 flex-1">
                ...
                @foreach ($topWorksWeekly as $i => $work)
                    <div class="group relative flex flex-col items-center cursor-pointer w-full max-w-[300px] sm:max-w-[330px] mx-auto">
                        ...
                        <div class="text-center w-full space-y-1.5 mt-2">
                            ...
                            <h3 class="font-display text-xl sm:text-2xl lg:text-3xl text-[#2F3AFF] leading-tight line-clamp-2-custom min-h-[3rem]">
                                #{{ $i + 1 }} {{ $work->title }}
                            </h3>
                        </div>
        ...
        <section class="space-y-10 sm:space-y-14">
            <div class="text-center">
                <h2 class="font-display text-3xl sm:text-5xl text-[#2F3AFF] tracking-normal">
                ...
                @foreach ($exploreCreators as $creator)
                    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 lg:gap-8">
                        ...
                        <div class="w-full lg:w-[480px] flex items-center gap-4 shrink-0">
                            <a href="{{ route('creator.show', $creator->slug ?? $creator->id) }}" class="w-24 h-24 sm:w-32 sm:h-32 aspect-square bg-gray-900 border-2 border-[#2F3AFF] shrink-0 overflow-hidden">
                            ...
                            <div class="flex-1 min-w-0 space-y-1">
                                ...
                                <div class="pt-1">
                                    @auth
                                        ...
                                        @else
                                            <button wire:click="toggleFollow({{ $creator->id }})" class="px-4 py-1 bg-[#D9FC28] hover:bg-[#FC00BB] text-[#2F3AFF] hover:text-[#D9FC28] font-display text-xs sm:text-sm active:translate-y-0.5 transition">
                        ...
                        <div class="w-full lg:flex-1 grid grid-cols-3 gap-3 sm:gap-4">
                            @foreach ($creator->preview_works as $work)
                                <a href="{{ route('work.show', $work->slug ?? $work->id) }}" class="group relative aspect-[4/3] bg-gray-900 border-2 border-[#FC00BB] block">
                                    <div class="absolute -top-1.5 -left-1.5 w-2.5 h-2.5 bg-[#D9FC28] border-2 border-[#FC00BB] z-30 pointer-events-none"></div>
                                    <div class="absolute -top-1.5 -right-1.5 w-2.5 h-2.5 bg-[#D9FC28] border-2 border-[#FC00BB] z-30 pointer-events-none"></div>
                                    <div class="absolute -bottom-1.5 -left-1.5 w-2.5 h-2.5 bg-[#D9FC28] border-2 border-[#FC00BB] z-30 pointer-events-none"></div>
                                    <div class="absolute -bottom-1.5 -right-1.5 w-2.5 h-2.5 bg-[#D9FC28] border-2 border-[#FC00BB] z-30 pointer-events-none"></div>
            ...
            <div class="text-center pt-4">
                <a href="{{ route('creators.more') }}" class="inline-block text-[#2F3AFF] hover:text-[#FC00BB] font-display text-sm sm:text-xl transition tracking-wider underline underline-offset-4">
                    Lihat lebih banyak →
                </a>
            </div>
        </section>
                    ...
                    <div class="flex flex-col {{ $isEven ? 'lg:flex-row-reverse' : 'lg:flex-row' }} items-center justify-between gap-8 lg:gap-14">
                        ...
                        <div class="w-full lg:w-1/2 relative flex justify-center">
                            ...
                            <div class="group relative w-full max-w-md aspect-[4/3] bg-gray-900 border-8 border-[#FC00BB] 
                                ...
                                <a href="{{ route('creator.show', $cSlug) }}" class="block w-full h-full overflow-hidden">
                                    ...
                                    @else
                                        <div class="w-full h-full bg-[#2F3AFF] flex items-center justify-center text-[#D9FC28] font-display text-3xl ">
                                            {{ $creator->name }}
                                        </div>
                                    @endif   
                                </a>
                                ...
                                <div class="absolute -bottom-4 {{ $isEven ? '-left-3 sm:-left-5' : '-right-3 sm:-right-5' }} z-30 
                                    ...
                                    <span class="font-display text-xl sm:text-3xl lg:text-4xl text-[#2F3AFF] tracking-wider whitespace-nowrap truncate px-0.5">
                                        {{ $creator->name }}
                                    </span>
                                </div>
                        ...
                        <div class="w-full lg:w-1/2 space-y-3 sm:space-y-4">
                            <div class="flex flex-wrap items-center justify-between gap-2 pb-1 sm:pb-2">
                                <div>
                                    <p class="font-sans text-[11px] sm:text-xs text-[#2F3AFF] font-bold tracking-wide">
                                        {{ $creator->creator_type ?? 'Riset Mengabdi Desa' }}
                                    </p>
                                ...
                                <button wire:click="toggleFollow({{ $creator->id }})" 
                                        class="px-4 py-1 bg-[#D9FC28] hover:bg-[#FC00BB] text-[#2F3AFF] hover:text-[#D9FC28] font-display text-xs sm:text-sm active:translate-y-0.5 transition">
