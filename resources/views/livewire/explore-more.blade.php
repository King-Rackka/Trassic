<div class="w-full">
    ...
    <div class="rimesa-container flex items-center justify-start sm:justify-center gap-2 overflow-x-auto pb-4 mb-8 no-scrollbar">
        <button wire:click="setCategory('')" 
                class="px-4 py-1.5 sm:px-5 sm:py-2 border-2 text-xs sm:text-sm font-display whitespace-nowrap transition-all duration-200 cursor-pointer
        ...
        @foreach ($categories as $cat)
            <button wire:click="setCategory('{{ $cat }}')" 
                    class="px-4 py-1.5 sm:px-5 sm:py-2 border-2 text-xs sm:text-sm font-display whitespace-nowrap transition-all duration-200 cursor-pointer
                    {{ $category === $cat ? 'bg-[#FC00BB] text-[#D9FC28] border-[#FC00BB]' : 'bg-[#2F3AFF] text-white border-[#FC00BB] hover:bg-[#D9FC28] hover:text-[#2F3AFF]' }}">
                {{ $cat }}
            </button>
        @endforeach
    </div>
    ...
    <div class="rimesa-container rimesa-grid items-stretch" wire:loading.class="opacity-60">
        ...
        @empty
            <div class="col-span-full text-center py-16 text-[#2F3AFF]">
                <p class="font-display text-lg sm:text-xl tracking-wider">Belum ada karya ditemukan.</p>
            </div>
        @endforelse
    </div>

    <div class="rimesa-container mt-10 sm:mt-14 flex justify-center">
        {{ $works->links() }}
    </div>
</div>