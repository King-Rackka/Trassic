<div class="w-full bg-grid-pattern min-h-screen font-sans" x-data="{ shareModalOpen: false, instaCopied: false, copied: false }">
    ...
    <style>
        ...
        /* Jarak tetap 80px dari tepi layar untuk konten (sampul tetap full width) */
        .profile-container {
            padding-left: 80px;
            padding-right: 80px;
        }
        ...
        /* Bingkai cover karya persis kotak (1:1) */
        .work-cover-square {
            width: 100%;
            aspect-ratio: 1 / 1;
            overflow: hidden;
    ...
    {{-- BAGIAN SAMPUL / COVER BANNER (FULL WIDTH TANPA PADDING) --}}
    <div class="relative w-full">
        <div class="w-full h-64 sm:h-80 md:h-[380px] lg:h-[420px] bg-gray-900 relative overflow-hidden z-10">
            {{-- Breadcrumb Navigation --}}
            <div class="absolute top-6 sm:top-8 left-4 sm:left-8 z-30 flex items-center gap-2 text-xs sm:text-sm font-sans tracking-[0.2em]">
        ...
        {{-- Foto Profil Lingkaran Sejajar Konten (Padding 80px) --}}
        <div class="profile-container w-full relative">
            <div class="absolute -bottom-16 sm:-bottom-20 z-30">
                <div class="w-32 h-32 sm:w-44 sm:h-44 rounded-full border-4 sm:border-[6px] border-white overflow-hidden bg-white shadow-xl flex items-center justify-center shrink-0">
                    @if ($creator->profile_image)
                        <img src="{{ asset('storage/' . $creator->profile_image) }}" 
                             alt="{{ $creator->name }}" 
                             class="w-full h-full object-cover">
                    @else
                        <div class="w-full h-full bg-[#2F3AFF] text-[#D9FC28] font-display text-4xl sm:text-5xl flex items-center justify-center ">
    ...
    <div class="profile-container w-full pt-20 sm:pt-24 pb-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-[#2F3AFF]/10 pb-6">
            ...
            <div class="flex items-center gap-2.5 sm:gap-3 shrink-0">
                @auth
                    @if(auth()->id() !== $creator->user_id)
                        <button wire:click="toggleFollow"
                                class="px-5 py-2 sm:px-6 sm:py-2.5 bg-[#D9FC28] hover:bg-[#cbf21d] text-[#2F3AFF] font-display text-xs sm:text-sm transition-all cursor-pointer">
                ...
                <button type="button" 
                        ...
                        class="w-9 h-9 sm:w-10 sm:h-10 bg-white border border-[#2F3AFF]/20 flex items-center justify-center text-[#2F3AFF] hover:bg-white hover:text-[#FC00BB] transition cursor-pointer" 
                        title="Bagikan">
                ...
                {{-- Tombol Opsi --}}
                <button class="w-9 h-9 sm:w-10 sm:h-10 bg-white border border-[#2F3AFF]/20 flex items-center justify-center text-[#2F3AFF] hover:text-[#FC00BB] transition cursor-pointer" title="Opsi">
        ...
        {{-- GRID UTAMA: BIO DI KIRI & KONTAK DI KANAN ATAS --}}
                    ...
                    <div class="space-y-2 sm:space-y-2.5 text-sm sm:text-base text-[#2F3AFF] font-semibold text-left">
                        ...
                        @if ($creator->phone)
                            <p class="flex items-center gap-2.5 sm:gap-3 truncate">
                                ...
                                @php
                                    $cleanPhone = preg_replace('/[^0-9]/', '', $creator->phone);
        ...
        <div class="flex items-center gap-2 mt-10 mb-8 overflow-x-auto pb-2">
            <span class="font-display text-xs sm:text-sm text-[#2F3AFF] tracking-wider mr-2">Filter:</span>

            <button wire:click="setSort('all')" 
                    class="px-4 py-1.5 border-2 border-[#2F3AFF] font-display text-xs transition-all cursor-pointer
            ...
            <button wire:click="setSort('newest')" 
                    class="px-4 py-1.5 border-2 border-[#FC00BB] font-display text-xs transition-all cursor-pointer
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 items-stretch" wire:loading.class="opacity-60">
                    ...
                    <a href="{{ route('work.show', $work->slug) }}" class="block w-full">
                        <div class="relative w-full aspect-square bg-gray-900 border-2 border-[#FC00BB] shrink-0">
                            ...
                            @if ($work->wasteDna && $work->wasteDna->sum('quantity') > 0)
                                <div class="absolute top-2 left-2 z-30 bg-[#D9FC28] text-[#2F3AFF] border border-[#2F3AFF] font-sans text-[7px] sm:text-[10px] font-extrabold px-1.5 sm:px-2 py-0.5 tracking-tight">
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    <p class="font-display text-base sm:text-lg text-gray-400">Belum ada karya untuk filter ini.</p>
                </div>
            @endforelse
        </div>
        ...
        @if ($works->count() >= 10)
            <div class="text-center mt-12 mb-6">
                <a href="{{ route('creator.works.more', $creator->slug) }}" 
                   class="inline-block font-display text-sm sm:text-lg text-[#2F3AFF] hover:text-[#FC00BB] tracking-wider underline underline-offset-4 transition-colors">
                    Lihat lebih banyak →
                </a>
            </div>
        @endif
            ...
            <div class="space-y-2">
                <div class="flex items-center gap-2 border-2 border-[#2F3AFF] p-1 bg-[#F8F8F8] relative">
                    ...
                    <button type="button" 
                            ...
                            "
                            class="px-4 py-2 bg-[#D9FC28] text-[#2F3AFF] hover:bg-[#2F3AFF] hover:text-[#D9FC28] font-display text-xs tracking-wider transition whitespace-nowrap cursor-pointer">
                            ...
                            <div class="w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-full border-4 sm:border-[5px] border-white overflow-hidden bg-white shrink-0 shadow-xl flex items-center justify-center">
                                @if ($creator->profile_image)
                                    <img src="{{ asset('storage/' . $creator->profile_image) }}" 
                                         ...
                                         class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full bg-[#2F3AFF] text-[#D9FC28] font-display text-3xl sm:text-4xl flex items-center justify-center ">
                                        {{ substr($creator->name, 0, 2) }}
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="flex flex-wrap items-center gap-x-8 sm:gap-x-10 gap-y-3 pt-2 text-white text-xs sm:text-sm font-semibold font-sans">
                        ...
                        @if ($creator->phone)
                            ...
                            <a href="https://wa.me/{{ $cleanPhone }}" 
                               target="_blank" 
                               rel="noopener noreferrer" 
                               class="flex items-center gap-2 hover:text-[#D9FC28] transition">
                                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
