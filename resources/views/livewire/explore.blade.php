<div class="w-full bg-grid-pattern min-h-screen py-8 sm:py-12">
    ...
    <section class="w-full py-8 sm:py-12 px-4 sm:px-8 max-w-7xl mx-auto text-center relative z-10">
        
        <h1 class="font-display text-3xl sm:text-5xl lg:text-6xl text-[#2F3AFF] uppercase mb-16 sm:mb-20 tracking-normal">
        ...
        <div class="w-full explore-banner-bg relative pt-[22%] sm:pt-[180px] pb-16 px-4 sm:px-12">
            ...
            <div class="flex items-center justify-start sm:justify-center gap-2 overflow-x-auto pb-4 mb-8 sm:mb-10 no-scrollbar px-2 max-w-6xl mx-auto relative z-10">
                <button wire:click="setCategory('')"
                        ...
                        {{ $category === '' ? 'bg-[#FC00BB] text-[#D9FC28] border-white' : 'bg-transparent text-white border-white hover:bg-[#D9FC28] hover:text-[#2F3AFF] hover:border-transparent' }}">
                    Semua
                </button>
                @foreach ($categories as $cat)
                    <button wire:click="setCategory('{{ $cat }}')"
                            class="px-3.5 py-1.5 sm:px-5 sm:py-2 border-2 text-xs sm:text-sm font-display uppercase whitespace-nowrap transition-all duration-200
                            {{ $category === $cat ? 'bg-[#FC00BB] text-[#D9FC28] border-white' : 'bg-transparent text-white border-white hover:bg-[#D9FC28] hover:text-[#2F3AFF] hover:border-transparent' }}">
                        {{ $cat }}
                    </button>
                @endforeach
            </div>
                ...
                @forelse ($works as $work)
                    ...
                    <div class="group relative flex flex-col justify-between transition-transform duration-200 hover:-translate-y-2 p-1 w-full h-full">
                        ...
                        <a href="{{ route('work.show', $work->slug) }}" class="block w-full">
                            {{-- BINGKAI FOTO DENGAN BORDER PINK ACCENT --}}
                            <div class="relative w-full aspect-square bg-gray-900 border-2 border-[#FC00BB]">
                                
                                {{-- 4 KOTAK KUNING LIME DI 4 POJOK SUDUT LUAR --}}
                        ...
                        {{-- DETAIL TEKS DI BAWAH BINGKAI (FLEX WITH GAP FOR LONG TITLES) --}}
                        <div class="mt-2.5 sm:mt-3 flex flex-col justify-between flex-grow text-center w-full">
                            
                            {{-- JUDUL DENGAN MAX 2 BARIS & TRUNCATE DOTS --}}
                            <h4 class="font-display text-xs sm:text-base text-white uppercase leading-tight tracking-wide line-clamp-2-custom min-h-[2rem] sm:min-h-[2.5rem]" title="{{ $work->title }}">
                                {{ $work->title }}
                            </h4>

                            <div class="mt-2 pt-1 border-t border-white/10">
                                <p class="font-sans text-[9px] sm:text-xs font-normal text-white uppercase truncate">
                                    {{ $work->creator->name ?? 'RIMESA 2026' }}
                                </p>
            ...
            <div class="text-center mt-10 sm:mt-12 pb-4 relative z-10">
                <a href="{{ route('explore.more', ['category' => $category]) }}"
                   class="inline-block text-[#D9FC28] font-display text-sm sm:text-xl hover:text-white transition uppercase tracking-wider underline underline-offset-4">
                    Lihat lebih banyak →
                </a>
            </div>
...
<section class="w-full py-12 sm:py-20 px-4 sm:px-8 max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row items-center justify-between gap-10 sm:gap-16">
        
        {{-- SISI KIRI: TEKS & TOMBOL SUBMIT RATA TENGAH --}}
        <div class="flex flex-col items-center justify-center text-center flex-1 space-y-6">
            <h2 class="font-display text-4xl sm:text-6xl text-[#2F3AFF] uppercase leading-tight tracking-tight">
            ...
            <div class="flex justify-center w-full">
                @auth
                    ...
                    class="inline-flex items-center justify-center bg-[#FC00BB] text-[#D9FC28] font-display text-base sm:text-xl px-8 sm:px-10 py-3.5 hover:bg-[#D9FC28] hover:text-[#2F3AFF] transition duration-200 uppercase tracking-wide cursor-pointer">
                        + Submit your work!
                    </a>
                @else
