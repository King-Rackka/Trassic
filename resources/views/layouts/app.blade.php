...
<header class="w-full border-b-2 border-[#2F3AFF] z-40 fixed top-0 left-0 bg-white transition-transform duration-300"
    ...
    <div class="w-full flex items-stretch justify-between h-16">
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-between gap-2 sm:gap-4 px-3 sm:px-6 lg:px-8 bg-white shrink-0 min-w-0">
            ...
            <button @click.stop="mobileMenuOpen = !mobileMenuOpen"
                    type="button"
                    class="lg:hidden bg-[#f2f4f7] hover:bg-gray-200 text-[#2F3AFF] p-2 rounded-lg transition focus:outline-none cursor-pointer shrink-0 ml-1">
            ...
            <div class="flex-1 flex items-center justify-end font-display shrink-0 relative min-w-0">
                @guest
                    <div class="flex items-center justify-end gap-2 w-full">
                        <a href="{{ route('login') }}" class="flex-1 max-w-[100px] text-center bg-[#D9FC28] text-[#2F3AFF] px-3 py-1 text-xs xl:text-sm hover:bg-opacity-90 transition whitespace-nowrap">Login</a>
                        <a href="{{ route('register') }}" class="flex-1 max-w-[110px] text-center bg-[#FC00BB] text-[#D9FC28] px-3 py-1 text-xs xl:text-sm hover:bg-opacity-90 transition whitespace-nowrap">Register</a>
                    </div>
                @else
    ...
    <div x-show="mobileMenuOpen"
        ...
        <div class="flex flex-col gap-3 font-display pt-4 border-t border-blue-400/50 w-full max-w-xs mx-auto">
            ...
            @else
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-2.5 bg-[#FC00BB] text-[#D9FC28] py-2.5 text-base font-display border-2 border-black cursor-pointer">
    ...
    @if (!($fullscreen ?? false))
        <footer class="w-full bg-[#F8F8F8] border-t-4 border-[#2F3AFF] shrink-0">
