<div class="w-full bg-grid-pattern min-h-screen pb-16 font-sans" x-data="{ activeFilter: @entangle('activeFilter'), shareModalOpen: false, instaCopied: false, copied: false }">
    ...
    <div class="relative w-full">
        ...
        <div class="profile-container w-full relative">
            <div class="absolute -bottom-16 sm:-bottom-20 z-30">
                <div class="w-32 h-32 sm:w-44 sm:h-44 rounded-full border-4 sm:border-[6px] border-white overflow-hidden bg-[#2F3AFF] flex items-center justify-center shrink-0 shadow-lg">
                    @if (isset($creator->profile_image) && $creator->profile_image && \Illuminate\Support\Facades\Storage::disk('public')->exists($creator->profile_image))
                        <img src="{{ asset('storage/' . $creator->profile_image) }}" 
                            ...
                            class="w-full h-full object-cover">
                    @else
                        <span class="text-[#D9FC28] font-display text-4xl sm:text-6xl">
    ...
    {{-- BAGIAN KONTEN PROFIL & KARYA --}}
    <div class="profile-container w-full pt-20 sm:pt-24">

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#2F3AFF]/10 pb-6 relative">
            <div>
                <h1 class="font-display text-3xl sm:text-5xl text-[#2F3AFF] tracking-normal leading-tight">
                    {{ $user->name }}
                </h1>
            </div>
        ...
        <div class="flex gap-8 sm:gap-16 py-4 text-[#2F3AFF]">
            <div>
                ...
                <span class="text-xs sm:text-sm text-gray-600 font-semibold ml-1">posts</span>
            </div>
            <div>
                <span class="font-extrabold text-base sm:text-xl">{{ number_format($followersCount ?? 0) }}</span>
                <span class="text-xs sm:text-sm text-gray-600 font-semibold ml-1">followers</span>
            </div>
            <div>
                <span class="font-extrabold text-base sm:text-xl">{{ number_format($followingCount ?? 0) }}</span>
                <span class="text-xs sm:text-sm text-gray-600 font-semibold ml-1">following</span>
            </div>
        </div>
                    ...
                    <div class="space-y-2 sm:space-y-2.5 text-sm sm:text-base text-[#2F3AFF] font-semibold text-left">
                        ...
                        @if ($creator->phone)
                            <p class="flex items-center gap-2.5 sm:gap-3">
                                ...
                                <a href="https://wa.me/{{ $creator->formattedWhatsappNumber() ?? preg_replace('/[^0-9]/', '', $creator->phone) }}" 
                                target="_blank" 
                                rel="noopener noreferrer" 
                                class="hover:underline hover:text-[#FC00BB] transition-colors">
                                    {{ $creator->phone }}
                                </a>
                            </p>
                        @endif
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mt-10 mb-8">
            <div class="flex flex-wrap items-center gap-2">
                <span class="font-display text-xs sm:text-sm text-[#2F3AFF] tracking-wider mr-1">Filter:</span>

                <button type="button" 
                        wire:click="setFilter('diposting')" 
                        class="px-2.5 sm:px-4 py-1 sm:py-1.5 border-2 border-[#2F3AFF] font-display text-[10px] sm:text-xs transition-all cursor-pointer {{ $activeFilter === 'diposting' ? 'bg-[#FC00BB] text-[#D9FC28] border-[#FC00BB]' : 'bg-[#2F3AFF] text-white' }}">
                    Diposting
                </button>

                <button type="button" 
                        wire:click="setFilter('favorit')" 
                        class="px-2.5 sm:px-4 py-1 sm:py-1.5 border-2 border-[#2F3AFF] font-display text-[10px] sm:text-xs transition-all cursor-pointer {{ $activeFilter === 'favorit' ? 'bg-[#FC00BB] text-[#D9FC28] border-[#FC00BB]' : 'bg-[#2F3AFF] text-white' }}">
                    Favorit
                </button>

                <button type="button" 
                        wire:click="setFilter('disukai')" 
                        class="px-2.5 sm:px-4 py-1 sm:py-1.5 border-2 border-[#2F3AFF] font-display text-[10px] sm:text-xs transition-all cursor-pointer {{ $activeFilter === 'disukai' ? 'bg-[#FC00BB] text-[#D9FC28] border-[#FC00BB]' : 'bg-[#2F3AFF] text-white' }}">
                    Disukai
                </button>

                <button type="button" 
                        wire:click="setFilter('dilaporkan')" 
                        class="px-2.5 sm:px-4 py-1 sm:py-1.5 border-2 border-[#2F3AFF] font-display text-[10px] sm:text-xs transition-all cursor-pointer {{ $activeFilter === 'dilaporkan' ? 'bg-[#FC00BB] text-[#D9FC28] border-[#FC00BB]' : 'bg-[#2F3AFF] text-white' }}">
                    Dilaporkan
                </button>
            </div>
...
<div class="relative w-full aspect-square bg-gray-900 border-2 border-[#FC00BB] shrink-0">
    ...
    @if ($work->wasteDna && $work->wasteDna->sum('quantity') > 0)
        <div class="absolute top-2 left-2 z-30 bg-[#D9FC28] text-[#2F3AFF] border border-[#2F3AFF] font-sans text-[7px] sm:text-[10px] font-extrabold px-1.5 py-0.5 pointer-events-none">
            {{ $work->wasteDna->sum('quantity') }}kg sampah terpakai
        </div>
    @endif

    {{-- TOMBOL EDIT KARYA (IKON PENSIL PINK) --}}
    @if ($isEditMode && $activeFilter === 'diposting')
        <a href="{{ route('works.edit', $work->id) }}" 
           class="absolute top-2 right-2 z-40 bg-[#FC00BB] text-[#D9FC28] p-1.5 sm:p-2 border border-[#FC00BB] shadow-lg hover:scale-110 hover:bg-[#2F3AFF] hover:text-[#D9FC28] transition-all cursor-pointer block"
           title="Edit Karya Ini">
            <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current text-white" viewBox="0 0 24 24">
                <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
            </svg>
        </a>
    @endif

    {{-- LINK & GAMBAR (DIKUNCI PAKAI ABSOLUTE INSET-0 UNTUK MENCEGAH LAYOUT BREAK) --}}
    <a href="{{ route('work.show', $work->slug) }}" class="absolute inset-0 w-full h-full overflow-hidden block">
        ...
        @endif
                    ...
                    <div class="mt-2.5 flex flex-col text-center w-full">
                        <h4 class="font-display text-xs sm:text-base text-[#2F3AFF] leading-tight tracking-wide line-clamp-2-custom min-h-[2rem] sm:min-h-[2.5rem]" title="{{ $work->title }}">
                            {{ $work->title }}
                        </h4>
            ...
            @empty
                <div class="col-span-full border-2 border-dashed border-[#2F3AFF]/20 p-8 sm:p-12 text-center bg-white/50 space-y-4 my-auto">
                    ...
                    <div>
                        <h3 class="font-display text-xl text-[#2F3AFF]">
                    ...
                    <div class="pt-2">
                        @if ($activeFilter === 'disukai')
                            <a href="{{ route('home') }}" class="inline-block px-5 py-2.5 bg-[#2F3AFF] text-white font-bold text-xs hover:bg-[#FC00BB] hover:text-[#D9FC28] transition">
                                Jelajahi Karya
                            </a>
                        @elseif ($activeFilter === 'diposting')
                            <a href="{{ route('works.create') }}" class="inline-block px-5 py-2.5 bg-[#D9FC28] text-[#2F3AFF] font-bold text-xs hover:bg-[#c8f01b] transition">
        ...
        <div class="mt-16 pt-10 border-t border-[#2F3AFF]/20">
            <h2 class="font-display text-3xl sm:text-5xl text-[#2F3AFF] mb-8 tracking-wide">
                Riwayat komentar
            </h2>
                                ...
                                <div class="w-10 h-10 sm:w-11 sm:h-11 shrink-0 border-2 border-black bg-white shadow-[2px_2px_0px_rgba(0,0,0,1)] overflow-hidden z-10">
                                    @if ($comment->user->creatorProfile?->profile_image)
                                        <img src="{{ asset('storage/' . $comment->user->creatorProfile->profile_image) }}" 
                                             ...
                                             class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full bg-[#2F3AFF] text-[#D9FC28] font-display flex items-center justify-center text-xs">
                                                    ...
                                                    <div class="w-8 h-8 sm:w-9 sm:h-9 shrink-0 border-2 border-black bg-white shadow-[2px_2px_0px_rgba(0,0,0,1)] overflow-hidden z-10">
                                                        @if ($reply->user->creatorProfile?->profile_image)
                                                            <img src="{{ asset('storage/' . $reply->user->creatorProfile->profile_image) }}" 
                                                                 ...
                                                                 class="w-full h-full object-cover">
                                                        @else
                                                            <div class="w-full h-full bg-[#2F3AFF] text-[#D9FC28] font-display flex items-center justify-center text-[10px]">
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2 border border-[#2F3AFF] p-1 bg-[#F8F8F8] relative">
                        ...
                        <button type="button" 
                                ...
                                "
                                class="px-4 py-2 bg-[#D9FC28] text-[#2F3AFF] hover:bg-[#2F3AFF] hover:text-[#D9FC28] font-bold text-xs tracking-wider transition whitespace-nowrap cursor-pointer">
